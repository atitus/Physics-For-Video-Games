...
# v1.22 17 January 2011 -- Danny Caballero
# Added labelColor attribute to PhysAxis and MotionMap
        ...
        try:
            ...
            if t > (self.interval * self.curMarker):
                ...
                if self.dropTime is not False:
                    ...
                    droptimeText = label(pos=self.obj.pos+epsilon, text='t='+str(t)+'s', height=10, box=False, color=labelColor)

                # Same with order label
                if self.labelMarkerOrder is not False:
                    label(pos=self.obj.pos-vector(0,self.markerScale*.5,0)+self.labelMarkerOffset, text=str(self.curMarker), height=10, box=False, color=labelColor)
        except TypeError as err:
            print("**********TYPE ERROR**********")
            print("Please check that you are not passing in a variable of the wrong type (e.g. a scalar as a vector, or vice-versa)!")
            print("******************************")
        ...
        while i<self.numLabels:      
            ...
            if updating:
                ...
                self.intervalLabels.append(
                    label(pos=intervalPos+self.labelShift, text=str(labelText),box=False,height = 8, color=labelColor) )
            i=i+1
