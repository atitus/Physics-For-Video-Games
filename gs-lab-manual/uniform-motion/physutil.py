...
# Reverted PhysGraph back to v1.23 because plotting functionaliy seed
# to be broken.
...
# Merged into version from Mike Schatz (recieved 28 Mar 2013)
...
class PhysGraph:
    ...
    #graphColors = [color.blue, color.red, color.green, color.yellow, color.orange, color.cyan, color.magenta]
    #USE THIS IN CLASS INIT, NOW USERS CAN CHANGE COLOR LIST IF THEY WANT

    def __init__(self, numPlots=1, title = None, xlabel = None, ylabel = None, backgroundColor = color.white, graphColors = [color.blue, color.red, color.green, color.yellow, color.orange, color.cyan, color.magenta],radius=0.015, dot=False, dot_size=8):
        ...
        try:
            ...
            colorForeground = [abs(a-b) for a,b in zip(backgroundColor,(1,1,1))];  #make foreground text opposite color of background
            self.graphDisplay = gdisplay(475,350, title = title, xtitle = xlabel, ytitle = ylabel, background = backgroundColor, foreground=colorForeground )
            ...
            for i in range(numPlots):
                self.graphs.append(gcurve(dot=dot, size=dot_size,color=graphColors[i%len(graphColors)]))
                #print self.graphs[-1].gcurve.__class__.__name__
                self.graphs[-1].gcurve.radius = radius;
        except TypeError as err:
            print("**********TYPE ERROR**********")
            print("Please check that you are not passing in a variable of the wrong type (e.g. a scalar as a vector, or vice-versa)!")
            print("******************************")
